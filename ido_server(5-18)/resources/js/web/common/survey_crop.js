	...
	return function () {
		...
		this.init = function(ele) {
			...
			$(ele.container)
				...
				.append('<input type="range"  name="range"  style="display:none;" value="'+ele.zoom.min+'" min="'+ele.zoom.min+'" max="'+ele.zoom.max+'" step="'+ele.zoom.steps+'"><div class="cropMain"></div>');
		...
		this.crop = function(width, height, type) {
			...
			return {
				file : canvas.getContext('2d').createImageData(canvas.width, canvas.height),
				string: canvas.toDataURL('image/png').replace(/^data:image\/(png|jpg);base64,/, ""),
			};
		...
		this.import = function() {
			var that = this;
			$('input[name=range]').css('display','');
			$('#titleDiv').append('<input type="file" id="importIMG"  style="display:none">');
